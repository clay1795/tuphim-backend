...
/**
 * @route   GET /api/comments/latest/all
 * @desc    Get latest comments from all movies (for TopComments component)
 * @access  Public
 */
router.get('/latest/all', async (req, res) => {
  try {
    ...
    logger.info(`Fetching latest comments from all movies - Limit: ${limit}`);
    ...
    // Calculate netVotes and replyCount for each comment since we're using lean()
    const latestCommentsWithStats = await Promise.all(
    ...
    logger.info(`Found ${latestCommentsWithStats.length} latest comments`);
    
    res.json({
      success: true,
      data: latestCommentsWithStats
    });
  } catch (error) {
    logger.error('Get latest comments error:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch latest comments',
      error: error.message
    });
  }
});
...
router.get('/top/:movieSlug', async (req, res) => {
  try {
    ...
    // Calculate netVotes and replyCount for each comment since we're using lean()
    const topCommentsWithStats = await Promise.all(
